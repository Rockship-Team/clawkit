...
func (g *GoogleSheets) Authenticate() (map[string]string, error) {
	...
	if clientID != "" {
		...
		fmt.Println("  Bước 1: Bật Google Sheets API...")
		OpenBrowser("https://console.cloud.google.com/apis/library/sheets.googleapis.com")
		fmt.Println("  → Nhấn Enable nếu chưa bật, sau đó quay lại đây.")
		fmt.Println()
		fmt.Println("  Bước 2: Đang mở Google Cloud Console để tạo OAuth credentials...")
		fmt.Println("  → Create Credentials → OAuth client ID → Desktop app")
		fmt.Printf("  → Redirect URI cần thêm: http://localhost:%d/callback\n", CallbackPort)
		fmt.Println()
